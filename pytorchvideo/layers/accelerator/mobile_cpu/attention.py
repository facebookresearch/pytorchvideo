...
class SqueezeExcitation(EfficientBlockBase):
    """
    Efficient Squeeze-Excitation (SE) block.

    The Squeeze-Excitation block is described in:
    ...
    def convert(self, input_blob_size, **kwargs):
        ...
        Args:
            input_blob_size (tuple): Size of the input blob.
        """
        if self.is_3d:
    ...
    def forward(self, x) -> torch.Tensor:
        """
        Forward pass through the Squeeze-Excitation (SE) block.

        Args:
            x (torch.Tensor): Input tensor.

        Returns:
            torch.Tensor: Output tensor after applying SE block.
        """
        out = self.se(x)
        return out
